/**
 * POST /api/whatsapp/evening
 * Send "Evening Vibe" WhatsApp messages to users
 *
 * Triggered by: Cron job (9:00 PM IST daily)
...
import { personalizeEveningWhatsApp } from "@/lib/whatsapp/personalizer";
import { eveningWhatsAppMessage } from "@/lib/whatsapp/templates";
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`[WhatsApp] evening: Starting delivery for ${today}`);

    // Fetch users eligible for evening WhatsApp
    ...
    console.log(`[WhatsApp] evening: Found ${users.length} eligible users`);
    ...
    for (const user of users) {
      try {
        ...
        const copy = await personalizeEveningWhatsApp(ctx);

        // Render message
        const message = eveningWhatsAppMessage({
          firstName: user.name || "there",
          headline: copy.headline,
          bodyText: copy.bodyText,
          challenge: "Try something bold tonight! 🔥",
        });
    ...
    console.log(
      `[WhatsApp] evening: Complete. Sent: ${sent}, Failed: ${failed}`,
    );
