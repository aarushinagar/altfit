/**
 * POST /api/whatsapp/daily-engagement
 * Send "Daily Engagement" WhatsApp messages to users
 *
 * Triggered by: Cron job (6:00 PM IST daily - evening time)
...
import { personalizeDailyEngagementWhatsApp } from "@/lib/whatsapp/personalizer";
import { dailyEngagementWhatsApp } from "@/lib/whatsapp/templates";
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      `[WhatsApp] daily-engagement: Starting delivery for ${today}`,
    );

    // Fetch users eligible for daily engagement WhatsApp
    const users = await prisma.user.findMany({
      where: {
        phone: { not: null },
        emailOptOut: false,
        wardrobeItemCount: { gte: 5 }, // At least 5 items for engagement
      },
      select: {
        id: true,
        phone: true,
    ...
    console.log(
      `[WhatsApp] daily-engagement: Found ${users.length} eligible users`,
    );
    ...
    for (const user of users) {
      try {
        ...
        const copy = await personalizeDailyEngagementWhatsApp(ctx);

        // Render message
        const message = dailyEngagementWhatsApp({
          firstName: user.name || "there",
          headline: copy.headline,
          bodyText: copy.bodyText,
          lookCount: 5, // Always 5 looks recommended
        });
    ...
    console.log(
      `[WhatsApp] daily-engagement: Complete. Sent: ${sent}, Failed: ${failed}`,
    );
    ...
    console.error(
      "[WhatsApp] daily-engagement route error:",
      err instanceof Error ? err.message : err,
    );
    return NextResponse.json(
      {
